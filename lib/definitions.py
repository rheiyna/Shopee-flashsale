...
def headerOutput(autoCheckout, autoOrder, chromedriver, session, urlTarget, options=[], justTitle=True):
    string = f'''
{Fore.LIGHTBLACK_EX}==========================================================
#              {Fore.RED}Shopee Flash-Sale Bot {Fore.LIGHTBLACK_EX}- {Fore.WHITE}By MRHRTZ                 {Fore.LIGHTBLACK_EX}#
# ====================================================== #
'''
    if not justTitle:
        string += f'''
...
def menu():
    ...
    list_menu = [
        inquirer.List(
            'main', message='Welcome to Flash Sale Bot, Select one..', choices=selector)
    ]
